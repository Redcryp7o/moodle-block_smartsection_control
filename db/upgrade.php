...
function xmldb_block_smartsection_control_upgrade(int $oldversion): bool {
    ...
    if ($oldversion < 2026071101) {
        ...
        //
        // Kept as $DB->execute(): both columns must change in a single statement.
        // set_field_select() writes one column at a time, and clearing modulename
        // first would make the WHERE clause stop matching the remaining rows.
        $DB->execute(
    ...
    if ($oldversion < 2026071201) {
        ...
        $DB->delete_records_select('block_smartsection_control', $orphanselect);

        // Remove history rows for deleted courses or sections.
        $DB->delete_records_select('block_smartsection_control_history', $orphanselect);

        // Remove per-student pacing rows for deleted courses or sections.
        $DB->delete_records_select('block_smartsection_user_unlocks', $orphanselect);
    ...
    if ($oldversion < 2026071209) {
        $softeventselect = "unlocktype = 'event' AND (locktype = 'soft' OR locktype IS NULL OR locktype = '')";
        $converted = $DB->count_records_select('block_smartsection_control', $softeventselect);
        if ($converted > 0) {
            $DB->set_field_select('block_smartsection_control', 'locktype', 'hard', $softeventselect);
        }
